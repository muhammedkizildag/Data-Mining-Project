...
Bu bölüm iki farklı durumu ayırır:

- `Kayıtlı final artifact modelleri`:
  repoda `models/` altında hazır gelen ve chatbot / smoke testleri tarafından kullanılan mevcut modellerdir.
- `Genişletilmiş model kümesi hızlı doğrulaması`:
  `SVM`, `ExtraTrees`, `AdaBoost` ve `LightGBM` entegrasyonundan sonra gerçek veri üzerinde yapılan hızlı train/test doğrulama sonuçlarıdır. Bunlar tam grid araması değil, entegrasyon ve karşılaştırma amaçlı güncel kontrol çıktılarıdır.
...
#### Kayıtlı final artifact sonucu

Repoda kayıtlı mevcut final model:

- `models/best_model_dropout_localized.pkl`
- model türü: `Random Forest`

Bu artifact için daha önce raporlanan 10-Fold CV sonuçları:
...
Yeni model ailesi entegrasyonundan sonra `%70/%30` split üzerinde alınan hızlı Macro F1 sonuçları:
...
Bu hızlı kontrolde en güçlü iki aday `XGBoost` ve `LightGBM` oldu. Bu bölüm tam grid araması sonucu değildir; entegrasyonun çalıştığını ve yeni model ailesinin rekabetçi davrandığını göstermek için eklenmiştir.
...
Bu hızlı kontrolde en güçlü iki aday `LightGBM` ve `XGBoost` oldu. `AdaBoost` aynı veri hattında diğer boosting yöntemlerine göre daha zayıf kaldı.
...
Dropout Localized ve OULAD v2 script'leri seçilen final model için çok sınıflı ROC/PR eğrileri ile learning curve grafikleri de üretir. ROC/PR tarafında her sınıf ayrı ayrı ve micro-average olarak raporlanır; learning curve tarafında ise eğitim örnek sayısı arttıkça train/validation Macro F1 davranışı gözlemlenir. Scriptler ayrıca `%80/%20` holdout karşılaştırmasını da konsola yazdırır.
...
- Genişletilmiş model kümesinin (`SVM`, `ExtraTrees`, `AdaBoost`, `LightGBM`) hızlı sonuçları `README` ve `GUNLUK.md` içinde yer alır; bunlar tam grid aramasının yerine geçen nihai benchmark değil, entegrasyon doğrulaması ve güncel karşılaştırma notudur.
